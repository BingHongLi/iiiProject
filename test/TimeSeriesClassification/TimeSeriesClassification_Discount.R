...
getClusterData <- function(cluster, method="k.means_8", rD="./data/forCrashCLUSTERPRICE1639.csv", wd="E:/R/TimeSeries"){
  setwd(wd)
  srcData = read.csv(rD, header=T, sep=",")  
  #srcData[raw of record in the cluster, col with game's name and time series]
  rawData <- srcData[which(srcData[clusterMethod] == cluster), c(1, 23:ncol(srcData))]  
...
getDiscount <- function(gameName, clusterNumber, day, clusterMethod="k.means_8", RDPath="./data/forCrashCLUSTERPRICE1639.csv", orgPDPath="./data/steamRegularPrice.csv", wd="E:/R/TimeSeries"){
  rawData <- getClusterData(clusterNumber, method=clusterMethod, rD=rawData, wd=wd)
  ...
  record <-c(day=day, discount=as.character(pDiscount))
  return(record)
}
